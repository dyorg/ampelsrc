...
class EyeDataGrid
{
	...
	// Default text
	const TXT_RESET = 'Atualizar';
	const TXT_NORESULTS = 'Nenhum registro encontrado!';
	...
	*/
	 ...
	 */
	public function setQuery($fields, $table, $primary = '', $where = '', $joins = '')
	{
	...
	*/
	public function setColumnHeader($column, $header, $width='', $buscar= true)
	{
		//$this->header[$column] = $header;
		//$this->header[$column]["width"] = $width;
		$this->header[] =  array('column' => $column,
								 'header' => $header,
								 'width' => $width,
								 'buscar' => $buscar);		
	}
	...
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{
			case self::STDCTRL_EDIT:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_edit . '" alt="Editar" title="Editar" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_DELETE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="Excluir" title="Excluir" class="tbl-control-image"></a>';
				break;
			default:
	...
	public function addCustomControl($type = self::CUSCTRL_TEXT, $action, $action_type = self::TYPE_ONCLICK, $text, $image_src = '')
	{
		...
		switch ($type)
		{
			case self::CUSCTRL_IMAGE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $image_src . '" alt="' . $text . '" title="' . $text . '" class="tbl-control-image"></a>';
				break;
	...
	*/
	private function buildHeader()
	{
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order)
				echo '<td class="tbl-header" '.$wi.' >' . $header; // Prevent the user from changing order
			else {
				...
				echo '<td class="tbl-header"  '.$wi.' ><a href="javascript:;" onclick="tblSetOrder(\'' . $t . '\', \'' . $order . '\')">' . $header . "</a>";
									...
									)))
			{
				...
				if ($buscar == true) 
					echo '<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'btn_busca.png" border="0" /></a></div>';
			}
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Registros encontrados <em>' . $this->row_count . '</em>';

		if ($this->row_count > 0)
			echo ', mostrando de <em>' . $first . '</em> até <em>' . $last . '</em>';
		...
		{
			if ($this->page > 1)
				echo '<a href="javascript:;" onclick="tblSetPage(1)"><img src="' . $this->image_path . 'arrow_first.gif" class="tbl-arrows" alt="&lt;&lt;" title="First Page"></a><a href="javascript:;" onclick="tblSetPage(' . ($this->page - 1) . ')"><img src="' . $this->image_path . 'arrow_left.gif" class="tbl-arrows" alt="&lt;" title="Página anterior"></a>';
			else
				echo '<img src="' . $this->image_path . 'arrow_first_disabled.gif" class="tbl-arrows" alt="&lt;&lt;" title="Primeira página"><img src="' . $this->image_path . 'arrow_left_disabled.gif" class="tbl-arrows" alt="&lt;" title="Próxima página">';
      ...
      for ($i = $startpage; $i <= $endpage; $i++)
			...
			if ($this->page < $pages)
				echo '<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ')"><img src="' . $this->image_path . 'arrow_right.gif" class="tbl-arrows" alt="&gt;" title="Próxima página"></a><a href="javascript:;" onclick="tblSetPage(' . $pages . ')"><img src="' . $this->image_path . 'arrow_last.gif" class="tbl-arrows" alt="&gt;&gt;" title="Última página"></a>';
			else
				echo '<img src="' . $this->image_path . 'arrow_right_disabled.gif" class="tbl-arrows" alt="&gt;" title="Próxima página"><img src="' . $this->image_path . 'arrow_last_disabled.gif" class="tbl-arrows" alt="&gt;&gt;" title="Última página">';
		}
	...
	*/
	public function printTable()
	{
		...
		$query = 'SELECT ' . $this->select_fields . ' FROM ' . $this->select_table . ' ' . $this->select_joins . ' ' . $filter;
		...
		if ($this->create_button)
			echo '<span class="tbl-create"><a ' . $this->create_button['Action'] . ' title="' . $this->create_button['Text'] . '"><img align="absmiddle" src="' . $this->image_path . $this->img_create . '" class="tbl-create-image"><label>' . $this->create_button['Text'] . '</label></a></span>';

		// Output the reset button
		if ($this->reset_button)
			echo '<span class="tbl-reset"><a href="javascript:;" onclick="tblReset()" title="' . $this->reset_button .'"><img align="absmiddle" src="' . $this->image_path . $this->img_reset . '" class="tbl-reset-image"><label>' . $this->reset_button .'</label></a></span>';
