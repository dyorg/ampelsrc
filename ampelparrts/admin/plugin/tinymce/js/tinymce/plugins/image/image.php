...
<html>
	<head>
		...
		<link href="bootstrap/file-uploader/fineuploader.css" rel="stylesheet" type="text/css"/>
		<script src="bootstrap/file-uploader/js/header.js"></script>
		<script src="bootstrap/file-uploader/js/util.js"></script>
		<script src="bootstrap/file-uploader/js/features.js"></script>
		<script src="bootstrap/file-uploader/js/promise.js"></script>
		<script src="bootstrap/file-uploader/js/button.js"></script>
		<script src="bootstrap/file-uploader/js/ajax.requester.js"></script>
		<script src="bootstrap/file-uploader/js/deletefile.ajax.requester.js"></script>
		<script src="bootstrap/file-uploader/js/handler.base.js"></script>
		<script src="bootstrap/file-uploader/js/handler.base.js"></script>
		<script src="bootstrap/file-uploader/js/window.receive.message.js"></script>
		<script src="bootstrap/file-uploader/js/handler.form.js"></script>
		<script src="bootstrap/file-uploader/js/handler.xhr.js"></script>
		<script src="bootstrap/file-uploader/js/paste.js"></script>
		<script src="bootstrap/file-uploader/js/uploader.basic.js"></script>
		<script src="bootstrap/file-uploader/js/dnd.js"></script>
		<script src="bootstrap/file-uploader/js/uploader.js"></script>
...
$(document).ready(function(){
	...
	$("#get-recent").bind("click", function () {
		...
		$.getJSON('recent.php',{}, function(returned){ 
			if(returned.success == 1){
				...
				$('#recent-images').empty();
				$('#recent-images').append('<center>Nenhum imagem recente encontrada.</center>');
			}
		});
	});
	...
	$("#refresh").bind("click", function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>Sem imagens no diretório.</center>');
			}
		});
	...
	$("#toggle-layout").bind("click", function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel"), toggle: 1}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>em imagens no diretório.</center>');
			}
		});
	...
	$(document).on('click', '#newfolder_btn', function () {
		if($('#newfolder_name').val() == ""){
			alert('Nome do diretório');
			return false;
		}
		
		$('#new-folder-msg').empty();
		$('#new-folder-msg').append('Criando...&nbsp;&nbsp;&nbsp;');
	...
	$(document).on('click', 'a.change-file', function () {
		...
		$(this).parent().parent().empty().append('<p>Salvando...</p>');
	...
	$(document).on('click', '#refresh-dirs', function () {
		...
		$('#select-dir-msg').append('Carregando...&nbsp;&nbsp;&nbsp;');
	...
	$(document).on('change', '#select-dir', function () {
		$('#select-dir-msg').empty();
		$('#select-dir-msg').append('Enviando...&nbsp;&nbsp;&nbsp;');
	...
	$(document).on('click', 'button#lib-back', function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<centerem imagens no diretório.</center>');
			}
		});
				...
				<div class="span12" style="margin-top: 20px;">
					...
					<div class="tabbable tabs-left">
						<ul class="nav nav-tabs" id="myTab">
							...
							<li><a href="#tab2" data-toggle="tab" id="get-lib"><i class="icon-folder-open"></i> Galeria</a></li>
							<?php }?>
							<?php if(CanAcessUploadForm()){?>
							<li><a href="#tab3" data-toggle="tab"><i class="icon-upload"></i> Enviar Imagem</a></li>
							<?php }?>
							<li><a href="#tab4" data-toggle="tab" id="get-recent"><i class="icon-time"></i> Últimas Enviadas</a></li>
						</ul>
...
var uploader = new qq.FineUploader({
        ...
        validation: {
            allowedExtensions: ['<?php echo implode("', '", explode(",", ALLOWED_IMG_EXTENSIONS));?>'],
            sizeLimit: <?php echo MBToBytes($upload_mb);?>
        },
        text: {
        ...
        callbacks: {
        	...
        	if(responseJSON.success == true){
        		$("#uploaded-images").append('<a style="margin: 9px; margin-right: 27px;" href="" class="img-thumbs" rel="' + responseJSON.file + '"><img src="timthumb.php?src=' + encodeURIComponent(responseJSON.file) + '&w=130&h=90" class="img-polaroid" width="130" height="90"></a>');
        	}
        }
        }
       ,
