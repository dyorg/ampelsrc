	...
	<script type="text/javascript">
		$("#dataGrid").flexigrid({
			...
			colModel : [
				...
				{display: 'Título', name : 'titulo', width : 400, sortable : true, align: 'left'},
				{display: 'Categoria', name : 'categoria', width : 200, sortable : true, align: 'left'},
				{display: 'Qtde. Imagens', name : 'qtdeimg', width : 80, sortable : true, align: 'center'},
				{display: 'Data Cadastro', name : 'data_cadastro', width : 150, sortable : true, align: 'center'},
				],
			buttons : [
		        {name: 'Editar', bclass: 'edit', onpress : doCommand},
		   		...
		   		{separator: true},
		   		{name: 'Imagem', bclass: 'image', onpress : doCommand},
		   		{separator: true}
		   		],		
			searchitems : [
				{display: 'ID', name : 'id'},
		...
		function doCommand(com, grid) {
			if (com == 'Editar') {
				...
				$('.trSelected', grid).each(function() {
					var id = $(this).attr('id');
					id = id.substring(id.lastIndexOf('row')+3);
					popup('../produtos_imagem/form.php?id_produto='+id, '<?=$popup?>', 550, 400);
				});
			}
		}		
